...
def label_objects(mask):
    return cc3d.connected_components(mask, connectivity=26) #26-way connectivity

def count_contacts(labelA, maskB):
    contact_count = 0
    for lbl in range(1, labelA.max() + 1):
        obj = (labelA == lbl)
        dilated = binary_dilation(obj, structure=structure, iterations=2) # iteration *2 total dilation 2 pixels
...
if __name__ == "__main__":
    ...
    total_placed_per_src = placed_counts.sum(axis=1).astype(float)  
